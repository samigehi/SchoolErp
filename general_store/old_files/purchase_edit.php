...
$qP = "SELECT pr_id, pr_items, pr_qty, pr_vat, pr_rate, bill_date, bill_no, pr_remark, supplier_name, st_items FROM maint_stock, maint_purchase WHERE st_id = pr_items AND pr_id = '$pr_id'";
...
<td class=td1 style="text-align:center;"><input size="12" type="text" value="<?php echo $supplier_name;?>" name="supplier_name"></td>
...
$data = "UPDATE maint_purchase SET pr_rate='".$pr_rate."', pr_qty='".$upr_qty."', supplier_name = '".$supplier_name."', bill_no = '".$bill_no."', bill_date= '".$bill_date."', pr_vat='".$pr_vat."', pr_remark = '".$pr_remark."' WHERE pr_id = '".$pr_id."'";
...
$po_data = "UPDATE maint_stock SET st_qty = st_qty - '".$po_qty."', rate = (SELECT (SUM(pr_rate * pr_qty)/SUM(pr_qty)) as 'rate' FROM maint_purchase where pr_items = '".$pr_items."') WHERE st_id = '".$pr_items."'";
...
$add_data = "UPDATE maint_stock SET st_qty = st_qty - '".$add_qty."', rate = (SELECT (SUM(pr_rate * pr_qty)/SUM(pr_qty)) as 'rate' FROM maint_purchase where pr_items = '".$pr_items."') WHERE st_id = '".$pr_items."'";
...
$less_data = "UPDATE maint_stock SET st_qty = st_qty - '".$less_qty."', rate = (SELECT (SUM(pr_rate * pr_qty)/SUM(pr_qty)) as 'rate' FROM maint_purchase where pr_items = '".$pr_items."') WHERE st_id = '".$pr_items."'";
