...
if (isset($_GET['fromdate']))
{
	...
	$sql = "SELECT pr_id, bill_date, bill_no, pr_items, pindt_no, pr_vat, pr_qty, pr_rate, supplier_name, pr_remark, unit, ((pr_qty * pr_rate)+((pr_qty * pr_rate)*(pr_vat))/100) as 'total', st_items FROM text_stock, text_purchase WHERE st_id = pr_items AND upper($field) LIKE '%$find%' AND bill_date BETWEEN '$fromdate' AND '$todate' ORDER BY pr_id";	
