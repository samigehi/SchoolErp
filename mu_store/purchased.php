...
for($i=0;$i<$limit;$i++) {
    ...
    $unit[$i] = mysql_real_escape_string($unit[$i]);
...
$query = "INSERT INTO mu_purchase (pr_date, bill_date, bill_no, supplier_name, pr_items, pr_qty, pr_rate, pr_vat, pr_remark, ip_1, by_user) VALUES 

('".$pr_date."', '".$bill_date."', '".$bill_no."', '".$supplier_name."', '".$pr_items[$i]."', '".$pr_qty[$i]."', '".$pr_rate[$i]."', '".$pr_vat[$i]."', '".$pr_remark[$i]."', '".$ip_1."', '".$by_user."')";
...
{
//update stock table//
$add_data = "UPDATE mu_stock SET st_qty = st_qty + '".$pr_qty[$i]."', rate = (SELECT SUM(pr_rate * pr_qty)/SUM(pr_qty) as 'rate' FROM mu_purchase where pr_items ='".$pr_items[$i]."') WHERE st_id='".$pr_items[$i]."'";
