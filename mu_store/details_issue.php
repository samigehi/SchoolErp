...
?>
	...
	<th class=th1 style="width:70px;">Items</th>
	<th class=th1 style="width:2px;">Exp date</th>
	<th class=th1 style="width:2px;">Unit</th>
	<th class=th1 style="width:2px;">Qty</th>	
	<th class=th1 style="width:2px;">Rate</th>
	<th class=th1 style="width:5px;">Total</th>
	<th class=th1 style="width:70px;">Description</th>	
	</tr>

	<?php
	$sql = "SELECT iss_id, iss_date, iss_items, iss_qty, customer_name, iss_remark, iss_rate, unit, date_format(exp_date,'%b, %Y') exp_date, st_items FROM mu_stock, mu_issue WHERE st_id = iss_items AND upper($field) LIKE '%$find%' AND iss_date BETWEEN '$fromdate' AND '$todate' ORDER BY iss_id";	
...
<td class=td1 style="text-align:left;"><?php echo "<a href=\"issue_edit.php?iss_id=$iss_id&fromdate=$fromdate&todate=$todate&field=$field&find=$find\" title='Edit issue - $st_items'>$st_items</a>";?></td>
	...
	<td class=td1 style="text-align:right;"><?php echo $iss_qty;?></td>
	...
	<td class=td1 colspan="9" align="right"><b>[ GRAND TOTAL ]</b></td><td class=td1><b><?php echo number_format($grand_total,2);?></b></td>
	<td class=td1></td>
	</tr>
	</table>
	</div>
