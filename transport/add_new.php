...
<td class=td1>
<select name="leave_type" onChange="autoSubmit();">
<option class=pink value="">select</option>
<?php
include ("connect.php");
...
?>
<tr class=tr1>
<td class=td1>C-off Available : </td>
<td class=td1>
<select name="leave_type" onChange="autoSubmit();">
<option class=pink value="">select</option>
<?php
$coff_sql=mysql_query("SELECT * FROM coff_leave_app where staff_name = '$staff_name' AND coff_status = 'Approved'");
while ($coff_data=mysql_fetch_array($coff_sql)) {
?>
<option class=pink value="<?php echo $coff_data['coff_date'];?>"><?php echo $coff_data['coff_date'];?></option>
